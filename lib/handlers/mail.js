...
/// `batch-update` / `batch-delete` carry a `_lookup` summary — the write-path message
/// locator's backend label and its {byIdHits, byIdMismatches, jxaFallbacks, notFound}
/// counters. That is diagnostic metadata about HOW the CLI found each message, and it is
/// deliberately unconditional there: the SQLite fast path degrades silently and correctly,
/// so the counters are the only way an operator can tell the fast path from the scan.
///
/// An agent is not that operator. It cannot act on the counters, and they ride along on
/// every batch call, so they are stripped at this boundary rather than at the CLI. Run the
/// CLI directly (or with MAIL_CLI_DEBUG=1, which adds per-message detail) to read them.
function stripLocatorTelemetry(result) {
  if (!result || typeof result !== "object" || Array.isArray(result)) return result;
  if (!("_lookup" in result)) return result;
  const { _lookup, ...rest } = result;
  return rest;
}
...
export async function handleMail(args, runCLI) {
  ...
  switch (args.action) {
    ...
    case "batch_update": {
      ...
      return stripLocatorTelemetry(await runCLI("mail-cli", batchArgs));
    }
