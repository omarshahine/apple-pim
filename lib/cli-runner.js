...
/**
 * Filesystem name of the helper bundle.
 *
 * It is "Apple PIM Helper.app" rather than "PIMHelper.app" because that string
 * is the one macOS shows the user: LaunchServices takes the name in a TCC
 * prompt and in System Settings > Privacy & Security from the bundle's
 * FILENAME, and deliberately ignores a CFBundleDisplayName that disagrees with
 * it (otherwise an app could masquerade as another). The bundle has always
 * declared CFBundleName = "Apple PIM Helper"; only the directory name moves
 * the needle.
 *
 * The bundle IDENTIFIER is untouched, which is what TCC keys grants on, so
 * this rename costs no permissions — scripts/build-helper-app.sh migrates an
 * old install with `mv`, which leaves the signature byte-identical.
 */
const HELPER_APP_NAME = "Apple PIM Helper.app";

/** What installs from before the rename are called. Resolved as a fallback so a host
 * that has not re-run the installer keeps working. */
const LEGACY_HELPER_APP_NAME = "PIMHelper.app";

/**
 * Resolve the helper .app path (overridable for testing / non-default installs).
 *
 * Prefers the current name and falls back to the legacy one only when that is
 * what is actually on disk. When neither exists it returns the current-name
 * path, so the "not installed" message names the bundle we would install.
 */
 ...
 * command line reads:
 *
 *     /bin/zsh "~/Applications/Apple PIM Helper.app/Contents/MacOS/../Resources/pim-helper.sh" ...
 *
 ...
 *
 * This is the pattern scripts/doctor.sh already uses. It matches every layout a resident
 * dispatcher can have: both bundle names (the current "Apple PIM Helper.app" and the
 * legacy "PIMHelper.app", which stays installed until the user re-runs the installer) and
 * both script locations (the launcher's ".../Contents/MacOS/../Resources/pim-helper.sh" and
 * the pre-launcher bundles still in the wild, whose script sat at "Contents/MacOS/pim-helper").
 *
 * It must NOT match the `open -W -a "~/Applications/Apple PIM Helper.app" --args ...` parent
 * this runner spawns. It doesn't: that argv names the bundle but never descends into it, so
 * it carries no "Contents/". This matters more than the widening does -- killing the parent
 * would abort a live call rather than clear a wedged one.
 */
export const HELPER_PROC_MARKER = "(Apple PIM Helper|PIMHelper)\\.app/Contents/.*pim-helper";
 ...
 *
 * The helper app is single-instance: if a previous invocation hung (the
...
/**
 * Run a CLI through the helper app via Launch Services.
 *
 * `open -W -a "Apple PIM Helper.app" --args <cli> <out> <err> <args...>` causes
    ...
    proc.on("close", (code) => {
      ...
      if (isFailure) {
        ...
        if (msg.includes("-1712")) {
          msg =
            "Apple PIM Helper did not respond (Launch Services error -1712). " +
 ...
 *
 * The returned runCLI auto-detects whether to route through the helper app:
 * on first call to each Calendar / Reminders / Contacts CLI it runs
 * `auth-status` directly. Unless the result is `authorized` or `fullAccess`,
 * and provided the helper is installed, all subsequent calls to that CLI are
 * routed through `open -W -a "Apple PIM Helper.app"`. The decision is cached per
 * runner instance so the probe runs at most once per CLI.
 *
 * When the probe saw `notDetermined` or `writeOnly`, the FIRST helper call for that CLI
 ...
 * @param {{
 *   timeoutMs?: number,
 *   runDirectImpl?: typeof runDirect,
 *   runViaHelperImpl?: typeof runViaHelper,
 *   helperExists?: () => boolean,
 * }} options - Options and injectable process seams.
...
export function createCLIRunner(
  ...
  async function probeRoute(cli) {
    ...
    // Probe directly. If TCC does not grant full read access to this process
    // tree, the helper is our only path to reads.
    const cliPath = join(binDir, cli);
    try {
      ...
      if (auth === "notDetermined" || auth === "writeOnly") {
        // The helper may need its own first grant or a Full Access upgrade.
