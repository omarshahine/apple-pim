...
**How to apply:** Mail uses a separate routing rule: direct access requires both Automation authorization and a readable Envelope Index; otherwise it uses the helper when installed. If a new EventKit/Contacts CLI is added, list it in `HELPER_ELIGIBLE_CLIS` in `lib/cli-runner.js` and add the corresponding usage string to `helper/Info.plist`. Do not bind Info.plist into the CLI Mach-O via linker `-sectcreate` — empirically tested, doesn't fix the responsible-process attribution. Related: see [[feedback_push_vs_pr]] for unrelated push/PR convention; nothing else cross-references yet.
