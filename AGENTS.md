...
`lib/cli-runner.js` auto-detects: on first call to each Calendar / Reminders / Contacts CLI it probes `auth-status` directly; if `notDetermined`, `denied`, or `writeOnly` AND the helper exists, it caches `route=helper` for that CLI for the remainder of the process. Helper invocations go through `/usr/bin/open -W -a PIMHelper.app --args <cli> <out> <err> <args...>` with `APPLE_PIM_BIN_DIR` exported so the dispatcher can find the CLI binary.
